...
- **An artist's "Top Songs"/"Similar Artists" child nodes must stay out of `collectSongsDeep`'s recursion into their parent Artist.** They're extra browsing entry points (`BrowserNode::CatArtistTopSongs`/`CatArtistSimilarArtists`, injected by `fetchChildren`'s `Artist` case), not part of the artist's discography — walking them as part of a "Play/Add whole artist" would pull in duplicate top tracks and drag in unrelated artists' entire catalogs (via Similar Artists → their own albums → songs). `collectSongsDeep` filters them out of a *parent*'s recursion (`isArtistSubCategory`); selecting either node directly still expands normally, since the filter only applies to iterating another node's children, not to the node itself.
...
- **`foo_navidrome.xcodeproj`'s link against `pfc` is a hand-typed filename, not a real cross-project target reference — it breaks silently whenever upstream renames the target.** `foo_navidrome`'s `PBXFileReference` for pfc is a plain `path = "libpfc.a"; sourceTree = BUILT_PRODUCTS_DIR;` entry (no `PBXReferenceProxy`/`PBXContainerItemProxy` into `pfc.xcodeproj`), and the target itself has an empty `dependencies = ()` — there is no formal Xcode dependency on pfc at all. It has only ever worked because Xcode's implicit-dependency resolution matches library names against product filenames of every target in the workspace. On 2026-09-22 this broke two ways at once, diagnosed from a real CI log (`gh run view --log-failed`), not guesswork:
  1. **No shared scheme was ever checked in** (`foo_navidrome.xcodeproj/xcshareddata/xcschemes/` didn't exist), so every headless `xcodebuild -workspace … -scheme foo_navidrome` in CI auto-generated a throwaway scheme — and at some point (likely a GH Actions `macos-14`/Xcode toolchain image update) that stopped reliably including `pfc` in the implicit-dependency build graph: zero `Libtool`/compile lines for it anywhere in the failing log. **Fix:** a committed shared `.xcscheme` with explicit `BuildActionEntries` for all 6 workspace targets (`foo_navidrome`, `pfc`, `foobar2000_SDK`, `foobar2000_SDK_helpers`, `shared`, `foobar2000_component_client`) plus `buildImplicitDependencies="YES"` — stop depending on autocreation. If a 7th shared-project dependency is ever added to the workspace, add its `BuildActionEntry` here too.
  2. **Upstream (`reupen/foobar2000-sdk-unmodified`, re-cloned fresh every CI run) renamed the target from `pfc-Mac` to plain `pfc`** (product `libpfc-Mac.a` → `libpfc.a`) at some point between our last green release (2026-09-16) and this one — confirmed by fetching their live `pfc/pfc.xcodeproj` and diffing the target's `name`/`productName`/`productReference` against our stale local sibling clone. Once fix (1) made Xcode actually *try* to build `pfc`, the link still failed (`ld: library 'pfc-Mac' not found`) because the filename our `PBXFileReference` hardcodes no longer matches what upstream produces. **Fix:** update the hardcoded `libpfc-Mac.a` → `libpfc.a` in `foo_navidrome.xcodeproj/project.pbxproj` (4 occurrences: `PBXBuildFile`, `PBXFileReference`, and their two membership-list entries) and the scheme's matching `BuildableName`/`BlueprintName`.
  - **If this breaks again after another upstream SDK sync**, don't assume it's the scheme — first fetch the live `pfc.xcodeproj` from `reupen/foobar2000-sdk-unmodified` and diff its `pfc` target's `name`/`productName` against what `foo_navidrome.xcodeproj`'s `PBXFileReference` hardcodes; a rename there is a silent, external, un-diffable (from our side) breaking change with no corresponding commit in *this* repo to point at.

- **This same upstream SDK sync also bumped `pfc-lite.h`'s minimum language standard from C++17 to C++20** (`#if _MSVC_LANG < 202002L` / `#if __cplusplus < 202002L`, was `201703L`) — every toolchain that compiles against real `pfc`/SDK headers needs `/std:c++20` or it's `error C1189: #error: C++20 please` before a single one of our own files even compiles. Four places hardcode the standard and all four had to move together: `Windows/foo_navidrome.vcxproj` and `tests/MediaEnrichmentTests.vcxproj`'s `<LanguageStandard>` (now `stdcpp20`), plus `scripts/win-build-local.sh` and `scripts/win-vm/build-mac.sh`'s clang-cl `/std:c++20` flag. `scripts/run-unit-tests.sh`'s two `-std=c++17`/`/std:c++17` flags (mac-test and Linux `make test`) were deliberately left alone — that path is genuinely SDK/pfc-free (no foobar2000 siblings needed), confirmed still green at C++17 through this exact incident, so there's nothing to bump there; don't cargo-cult the change onto it.

- **Same SDK sync a third time: `pfc.lib`'s ARM64EC build now needs `pfc_crashHook`/`pfc_winFormatSystemErrorMessageHook` (extern "C"), but nothing in the SDK's own `shared` project defines them** — `foobar2000/shared/wrap_pfc_hooks.h` has the definitions, upstream just never wired an `#include` for it into any of their own `.cpp` files, so `shared.vcxproj`'s ARM64EC link fails with unresolved externals (x64/Win32 don't reference the EC-only symbols, so they're unaffected). We can't edit the SDK's own vcxproj sources (checked out fresh every run, not vendored here) so `build-windows.yml` patches around it post-checkout: appends `#include "wrap_pfc_hooks.h"` to the freshly-staged `foobar2000/shared/stdafx.cpp` (a single-TU file, matching the header's own single-definition contract) in its own step, right after "Move SDK subdirs into the expected sibling layout". `release.yml` (macOS) doesn't need the same patch — the header is `#ifdef _WIN32`-guarded. If upstream ever fixes this themselves, this step becomes a silent no-op-until-it-isn't (a second `#include` of an already-included header is harmless) unless they also target `stdafx.cpp` specifically for their own include, which would then double the symbol definitions and fail to link — if `build-windows.yml` starts failing on this again post an SDK bump, check upstream first before re-patching blind.
