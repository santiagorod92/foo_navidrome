...
- **The C++20 bump also broke every `u8"…"` literal in `tests/MediaEnrichmentLogicTests.cpp`** — C++20 makes `u8"…"` a `const char8_t[]`, not `const char[]` like C++17 (a deliberate, well-known C++20 breaking change, correctly enforced by MSVC once `/std:c++20` took effect), so every `u8"café"`/`u8"中文…"` test literal stopped implicitly converting to the `std::string`/`const char*` parameters our own helpers take (`error C2664`/`C2088`/`C2446`/`C2440`, ~10 call sites). Fix: drop the `u8` prefix — plain `"café"` is still UTF-8 bytes as `const char[]` (the file is saved UTF-8, `/utf-8` is set project-wide), functionally identical to the old `u8"café"` pre-C++20 and exactly what the `std::string`-based API surface expects. If a future edit reaches for `u8"…"` here for "correctness," it'll silently reintroduce this — there's no legitimate use for actual `char8_t` anywhere in this codebase, everything is `std::string`/`char`.
