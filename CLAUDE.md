...
- **`foo_navidrome.xcodeproj/xcshareddata/xcschemes/foo_navidrome.xcscheme` must stay checked in.** The `foo_navidrome` target has an empty `dependencies = ()` in its `project.pbxproj` — it has always relied on Xcode's implicit-dependency resolution (scanning the Frameworks build phase against the workspace's other projects: `pfc-Mac`, `foobar2000_SDK`, `foobar2000_SDK_helpers`, `shared`, `foobar2000_component_client`) to build those first. With no scheme committed, every headless `xcodebuild -workspace … -scheme foo_navidrome` in CI had to auto-generate a throwaway scheme on the fly — until 2026-09-22 that happened to work, then started failing every run with `ld: library 'pfc-Mac' not found` (compiles clean, dies at final link — the target silently never got scheduled to build). No corresponding change in this repo or the SDK bundle explains the flip; likely a GH Actions `macos-14` runner/Xcode toolchain update changed autocreated-scheme behavior. Fix: a real shared `.xcscheme` with explicit `BuildActionEntries` for all 6 workspace targets (`buildImplicitDependencies="YES"` too, belt-and-suspenders) — pin it once, stop depending on autocreation. If a 7th shared-project dependency is ever added to the workspace, add its `BuildActionEntry` here too, or it'll silently ride on implicit resolution again.
